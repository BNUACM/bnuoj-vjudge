...
void WHUJudger::login() {
    ...
    string post = "origURL=%2Fland&password=&passEnc=" + encryptedPassword + "&username=" + info->GetUsername() + "&seed=" + seed + "&remember=1";
...
int WHUJudger::submit(Bott * bott) {
    ...
    string post = "lang=" + bott->Getlanguage() + "&problem_id=" + bott->Getvid() + "&source=" + escapeURL(bott->Getsrc()) + "&submit=Submit";
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
    performCurl();
...
Bott * WHUJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, ((string)"http://acm.whu.edu.cn/land/status?username=" + info->GetUsername() + "&problem_id=" + bott->Getvid() + "&language=" + bott->Getlanguage()).c_str());
        performCurl();
        ...
        // get first row
        if (!RE2::PartialMatch(html, "(?s)(<tr class=\"tro\">.*?</tr>)", &status)) {
        ...
        if (isFinalResult(result)) {
            ...
            if (!RE2::PartialMatch(status,
                    "(?s)<td.*?>([0-9]*).*?<font.*?</font>.*?<td.*?>([0-9]*).*?<td.*?>([0-9]*)",
...
string WHUJudger::getCEinfo(Bott * bott) {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("http://acm.whu.edu.cn/land/source/info?source_id=" + bott->Getremote_runid()).c_str());
    performCurl();
