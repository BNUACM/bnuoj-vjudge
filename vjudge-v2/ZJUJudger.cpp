...
void ZJUJudger::login() {
    ...
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, "http://acm.zju.edu.cn/onlinejudge/login.do");
    string post = "handle=" + escapeURL(info->GetUsername()) + "&password=" + escapeURL(info->GetPassword());
...
int ZJUJudger::submit(Bott * bott) {
    ...
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, "http://acm.zju.edu.cn/onlinejudge/submit.do");
    string post = (string)"problemCode=" + bott->Getvid() + "&languageId=" + bott->Getlanguage() + "&source=" + escapeURL(bott->Getsrc());
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
    performCurl();
...
Bott * ZJUJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, ("http://acm.zju.edu.cn/onlinejudge/showRuns.do?contestId=1&idStart=" + bott->Getremote_runid() + "&idEnd=" + bott->Getremote_runid()).c_str());
        performCurl();
        ...
        // get first row
        if (!RE2::PartialMatch(html, "(?s)(<tr class=\"rowOdd\">.*?</tr>)", &status)) {
            continue;
        }
        
        // get result
        if (!RE2::PartialMatch(status, "(?s)<td class=\"runJudgeStatus\".*?<span.*?>(.*?)</span>", &result)) {
        ...
        if (isFinalResult(result)) {
            ...
            if (!RE2::PartialMatch(status,
                    "(?s)<td class=\"runTime\".*?>(.*?)</td>.*<td class=\"runMemory\".*?>(.*?)</td>",
            ...
            if (result == "Compile Error") {
                // hack for ZJU, don't know why its submission id is inconsistent with judge protocol
                if (!RE2::PartialMatch(status, "(?s)showJudgeComment\\.do\\?submissionId=([0-9]*)", &submission_id_for_ce)) {
...
string ZJUJudger::getCEinfo(Bott * bott) {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("http://acm.zju.edu.cn/onlinejudge/showJudgeComment.do?submissionId=" + submission_id_for_ce).c_str());
    performCurl();
...
string ZJUJudger::convertResult(string result) {
    ...
    if (result.find("Segmentation Fault") != string::npos) return "Runtime Error";
    ...
    if (result.find("Non-zero Exit Code") != string::npos) return "Runtime Error";
    if (result.find("Floating Point Error") != string::npos) return "Runtime Error";
