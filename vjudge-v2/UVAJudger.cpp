...
Bott * UVAJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, "http://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=9");
        performCurl();
...
string UVAJudger::getCEinfo(Bott * bott) {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("http://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=9&page=show_compilationerror&submission=" + bott->Getremote_runid()).c_str());
    performCurl();
