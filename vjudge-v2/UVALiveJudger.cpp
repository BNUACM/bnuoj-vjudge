...
string UVALiveJudger::getLoginHiddenParams() {
    prepareCurl();    
    curl_easy_setopt(curl, CURLOPT_URL, "https://icpcarchive.ecs.baylor.edu/index.php");
    performCurl();
    
    string html = loadAllFromFile(tmpfilename);
    string form;
    // login form
    if (!RE2::PartialMatch(html, "(?s).*(<form.*?mod_loginform.*?</form>)", &form)) {
...
void UVALiveJudger::login() {
    ...
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_REFERER, "https://livearchive.onlinejudge.org/");
    curl_easy_setopt(curl, CURLOPT_URL, "https://icpcarchive.ecs.baylor.edu/index.php?option=com_comprofiler&task=login");
    string post = hiddenParams + "username=" + escapeURL(info->GetUsername()) + "&passwd=" + escapeURL(info->GetPassword()) + "&remember=yes&Submit=Login";
...
int UVALiveJudger::submit(Bott * bott) {
    ...
    if (html.find("You have to select a programming language.") != string::npos ||
            ...
            html.find("You have to paste or upload some source code.") != string::npos ||
            html.find(" You are not authorised to view this resource.") != string::npos) return SUBMIT_OTHER_ERROR;
...
Bott * UVALiveJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, "https://icpcarchive.ecs.baylor.edu/index.php?option=com_onlinejudge&Itemid=9");
        performCurl();
        ...
        if (html.find("<b>One or more following ERROR(s) occurred.") != string::npos ||
                ...
                !RE2::PartialMatch(html, "(?s)(<tr class=\"sectiontableentry1\">.*?</tr>)", &status)) {
            throw Exception("Failed to get status row.");
        }
        
        // get result
        if (!RE2::PartialMatch(status, "(?s)<td>([0-9]*?)</td>.*?<td>.*?<td>(.*?)</td>", &runid, &result)) {
        ...
        if (isFinalResult(result)) {
            ...
            if (!RE2::PartialMatch(status,
                                   "(?s)<td>([0-9]*?)</td>.*?<td>.*?<td>(.*?)</td>.*?<td>.*?<td>(.*?)</td>.*?<td>(.*?)</td>",
...
string UVALiveJudger::getCEinfo(Bott * bott) {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("https://icpcarchive.ecs.baylor.edu/index.php?option=com_onlinejudge&Itemid=9&page=show_compilationerror&submission=" + bott->Getremote_runid()).c_str());
    performCurl();
