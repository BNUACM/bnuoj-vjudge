...
string CCJudger::getLoginHiddenParams() {
    ...
    string form;
    // login form
    if (!RE2::PartialMatch(html, "(?s)(<form.*?user-login-form.*?</form>)", &form)) {
        throw Exception("Failed to get hidden params.");
    }
    string key, value, result = "";
    // get all hidden params
    re2::StringPiece formString(form);
    while (RE2::FindAndConsume(&formString, "(?s)<input type=\"hidden\".*?name=\"(.*?)\".*?value=\"(.*?)\"", &key, &value)) {
...
void CCJudger::login() {
    ...
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, "http://www.codechef.com/node?destination=node");
    string post = hiddenParams + "name=" + escapeURL(info->GetUsername()) + "&pass=" + escapeURL(info->GetPassword()) + "&submit.x=0&submit.y=0";
...
vector< pair<string, string> > CCJudger::getSubmitHiddenParams(string code) {
    prepareCurl();    
    curl_easy_setopt(curl, CURLOPT_URL, ((string)"http://www.codechef.com/submit/" + code).c_str());
    performCurl();
...
int CCJudger::submit(Bott * bott) {
    ...
    for (vector< pair<string, string> >::iterator it = hiddenParams.begin(); it != hiddenParams.end(); ++it) {
    ...
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ((string)"http://www.codechef.com/submit/" + bott->Getvid()).c_str());
    ...
    // check submit status
    string html = loadAllFromFile(tmpfilename);
    if (html.find("<li>You can\'t submit in this language for this problem!</li>") != string::npos) {
...
Bott * CCJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, ((string)"http://www.codechef.com/get_submission_status/" + bott->Getremote_runid()).c_str());
        performCurl();
        ...
        if (isFinalResult(result)) {
            ...
            if (result != "Compile Error") {
                ...
                memory_used = "0";
                prepareCurl();
                curl_easy_setopt(curl, CURLOPT_URL, ((string)"http://www.codechef.com/submissions?handle=" + escapeURL(info->GetUsername()) + "&pcode=" + escapeURL(bott->Getvid())).c_str());
                performCurl();
                html = loadAllFromFile(tmpfilename);
                string status;
                if (!RE2::PartialMatch(html, "(?s)(<tr class=\"kol.*?<td width=\"60\">" + bott->Getremote_runid() + "</td>.*?</tr>)", &status)) {
...
string CCJudger::getCEinfo(Bott * bott) {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("http://www.codechef.com/view/error/" + bott->Getremote_runid()).c_str());
    performCurl();
