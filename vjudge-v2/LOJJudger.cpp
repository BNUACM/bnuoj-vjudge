...
void LOJJudger::login() {
    ...
    string post = "myuserid=" + escapeURL(info->GetUsername()) + "&mypassword=" + escapeURL(info->GetPassword()) + "&Submit=Login";
...
int LOJJudger::submit(Bott * bott) {
    ...
    // check submit status
    string html = loadAllFromFile(tmpfilename);
    if (html.find("<script>location.href='volume_usersubmissions.php'</script>") == string::npos) {
...
Bott * LOJJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, "http://www.lightoj.com/volume_usersubmissions.php");
        performCurl();
        ...
        // get first row
        if (!RE2::PartialMatch(html, "(?s)(<tr class=\"newone\">.*?</tr>)", &status)) {
            throw Exception("Failed to get status row.");
        }
        
        // get result
        if (!RE2::PartialMatch(status, "(?s)sub_id=([0-9]*).*?<div.*?>(.*?)</div>", &runid, &result)) {
        ...
        if (isFinalResult(result)) {
            ...
            if (result != "Compile Error") {
                ...
                if (!RE2::PartialMatch(status,
                        "(?s)sub_id=([0-9]*).*?<td.*?<td.*?<td.*?<td.*?>(.*?)</td.*?<td.*?>(.*?)</td>",
...
string LOJJudger::getCEinfo(Bott * bott) {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("http://www.lightoj.com/volume_showcode.php?sub_id=" + bott->Getremote_runid()).c_str());
    performCurl();
