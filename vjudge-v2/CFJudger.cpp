...
string CFJudger::getActionUrl() {
  ...
  string html = loadAllFromFile(tmpfilename);
  string url;
  if (!RE2::PartialMatch(html, "<form class=.*submit-form.*action=\"(.*?)\"",
                         &url)) {
...
int CFJudger::submit(Bott * bott) {
  ...
  curl_easy_setopt(
      curl, CURLOPT_URL,
      ((string) "http://codeforces.com/problemset/submit?csrf_token=" +
          csrf).c_str());
...
string CFJudger::getVerdict(string contest, string runid) {
  ...
  curl_easy_setopt(
      curl, CURLOPT_URL,
      ((string) "http://codeforces.com/contest/" + contest + "/submission/" +
          runid).c_str());
  performCurl();
