...
void SCUJudger::login() {
    ...
    string post = "back=2&submit=login&id=" + escapeURL(info->GetUsername()) +"&password=" + escapeURL(info->GetPassword());
...
int SCUJudger::submit(Bott * bott) {
    ...
    string post = (string)"problemId=" + bott->Getvid() + "&submit=Submit&validation=" + code + "&language=" + escapeURL(bott->Getlanguage()) + "&source=" + escapeURL(bott->Getsrc());
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
    performCurl();
...
Bott * SCUJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, ("http://cstest.scu.edu.cn/soj/solutions.action?userId=" + escapeURL(info->GetUsername()) + "&problemId=" + bott->Getvid()).c_str());
        performCurl();
        ...
        // get first row
        if (!RE2::PartialMatch(html, "(?s).*<table.*?<tr.*?(<tr.*?<td height=\"44\">.*?</tr>)", &status)) {
...
string SCUJudger::getCEinfo(Bott * bott) {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("http://cstest.scu.edu.cn/soj/judge_message.action?id=" + bott->Getremote_runid()).c_str());
    performCurl();
 ...
 */
void SCUJudger::loadImage(string filename, int &width, int &height, int *& bitmap) {
 ...
 */
char SCUJudger::getNXY(int n, int x, int y, int width, int height, int * bitmap) {
