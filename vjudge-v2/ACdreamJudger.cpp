...
int ACdreamJudger::submit(Bott * bott) {
    ...
    string post = (string) "pid=" + bott->Getvid() + "&lang=" + escapeURL(bott->Getlanguage()) + "&code=" + escapeURL(bott->Getsrc());
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
...
Bott * ACdreamJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, ((string)"http://acdream.info/status?name=" + escapeURL(info->GetUsername()) + "&pid=" + bott->Getvid() + "&lang=" + bott->Getlanguage()).c_str());
        performCurl();
        ...
        // get result
        if (!RE2::PartialMatch(status, "(?s)<td rid=\"([0-9]*)\".*?>(.*?)</td>", &runid, &result)) {
...
string ACdreamJudger::convertResult(string result) {
    ...
    if (result.find("Dangerous Code") != string::npos) return "Restricted Function";
