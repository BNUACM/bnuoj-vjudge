...
void HRBUSTJudger::login() {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, "http://acm.hrbust.edu.cn/index.php?m=User&a=login");
    string post = (string)"m=User&a=login&ajax=1&user_name=" + escapeURL(info->GetUsername()) + "&password=" + escapeURL(info->GetPassword());
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
    performCurl();
    
    string html = loadAllFromFile(tmpfilename);
    if (html.find("{\"status\":0") != string::npos ||
            html.find("<div class='body'>Sorry!Login Error,Please Retry!</div>") != string::npos) {
...
int HRBUSTJudger::submit(Bott * bott) {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, "http://acm.hrbust.edu.cn/index.php?m=ProblemSet&a=postCode");
    string post = (string)"jumpUrl=&language=" + bott->Getlanguage() + "&problem_id=" + bott->Getvid() + "&source_code=" + escapeURL(bott->Getsrc());
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
    performCurl();
    
    string html = loadAllFromFile(tmpfilename);
    if (html.find("<html") == string::npos ||
            html.find("var is_login=\"\";") != string::npos) return SUBMIT_OTHER_ERROR;
...
Bott * HRBUSTJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, ((string)"http://acm.hrbust.edu.cn/index.php?m=Status&a=showStatus&problem_id=" + escapeURL(bott->Getvid()) + "&user_name=" + escapeURL(info->GetUsername()) + "&language=" + escapeURL(bott->Getlanguage())).c_str());
        performCurl();
        ...
        if (html.find("var is_login=\"\";") != string::npos ||
                !RE2::PartialMatch(html, "(?s)<table class=\"ojlist\".*?<tr.*?(<tr.*?</tr>)", &status)) {
            throw Exception("Failed to get status row.");
        }
        
        // get result
        if (!RE2::PartialMatch(status, "(?s)<td.*?<td>([0-9]*).*?<td.*?<td.*?>(.*?)</td>", &runid, &result)) {
...
string HRBUSTJudger::getCEinfo(Bott * bott) {
    ...
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("http://acm.hrbust.edu.cn/index.php?m=Status&a=showCompileError&run_id=" + bott->Getremote_runid()).c_str());
    performCurl();
