...
void SYSUJudger::login() {
    ...
    string post = "username=" + escapeURL(info->GetUsername()) + "&password=" + escapeURL(info->GetPassword()) + "&lsession=1";
...
int SYSUJudger::submit(Bott * bott) {
    ...
    string post = (string)"cid=0&language=" + bott->Getlanguage() + "&pid=" + bott->Getvid() + "&source=" + escapeURL(bott->Getsrc());
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
    performCurl();
...
Bott * SYSUJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, "http://soj.me/action.php?act=QueryStatus");
...
string SYSUJudger::getCEinfo(Bott * bott) {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("http://soj.me/compileresult.php?sid=" + bott->Getremote_runid()).c_str());
    performCurl();
...
string SYSUJudger::convertResult(string result) {
    ...
    if (result.find("Other") != string::npos) return "Judge Error";
    if (result.find("Restrict Function") != string::npos) return "Restricted Function";
    return trim(result);
}