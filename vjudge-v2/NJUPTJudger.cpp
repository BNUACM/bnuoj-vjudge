...
void NJUPTJudger::login() {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, "http://acm.njupt.edu.cn/acmhome/login.do");
    string post = (string)"userName=" + info->GetUsername() + "&password=" + info->GetPassword();
...
Bott * NJUPTJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, ((string)"http://acm.njupt.edu.cn/acmhome/showstatus.do").c_str());
        string post = (string) "problemId=" + bott->Getvid() + "&languageS=" + escapeURL(bott->Getlanguage()) + "&userName=" + escapeURL(info->GetUsername()) + "&resultS=All";
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
        performCurl();
        ...
        // get first row
        if (!RE2::PartialMatch(html, "(?s)<table.*?</thead>.*?(<tr.*?</tr>)", &status)) {
            throw Exception("Failed to get status row.");
        }
        
        // get result
        if (!RE2::PartialMatch(status, "(?s)method=showdetail.*?value=\"(.*?)\".*?<b acc=\"acc\"></b>\\s*(.*?)\\s*<b acc=\"acc\"></b>", &runid, &result)) {
        ...
        if (isFinalResult(result)) {
            ...
            if (result == "Accepted") {
                ...
                if (!RE2::PartialMatch(status, "(?s)([0-9]*)<b ms=\"ms\"></b>MS.*?([0-9]*)<b k=\"k\"></b>K", &time_used, &memory_used)) {
...
string NJUPTJudger::convertResult(string result) {
    if (result.find("Time Limit Exceed") != string::npos) return "Time Limit Exceed";
    if (result.find("Memory Limit Exceed") != string::npos) return "Memory Limit Exceed";
    if (result.find("Output Limit Exceed") != string::npos) return "Output Limit Exceed";
    ...
    if (result.find("Judge Delay") != string::npos) return "Judge Error";
    if (result.find("Judge Error") != string::npos) return "Judge Error";
    return trim(result);
}
...
string NJUPTJudger::getCEinfo(Bott * bott) {
    ...
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("http://acm.njupt.edu.cn/acmhome/compileError.do?id=" + bott->Getremote_runid()).c_str());
    performCurl();
    ...
    if (!RE2::PartialMatch(buffer, "(?s)Details of Compile Error</strong></h2></div>(.*)<div align=\"center\">", &result)) {
        return "";
    }
