...
void AizuJudger::login() {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, "http://judge.u-aizu.ac.jp/onlinejudge/index.jsp");
    string post = (string)"loginUserID=" + info->GetUsername() + "&loginPassword=" + escapeURL(info->GetPassword()) + "&submit=Sign+in";
...
int AizuJudger::submit(Bott * bott) {
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, "http://judge.u-aizu.ac.jp/onlinejudge/servlet/Submit");
    string post = (string) "userID=" + escapeURL(info->GetUsername()) + "&password=" + escapeURL(info->GetPassword()) + 
            "&problemNO=" + bott->Getvid() + "&language=" + escapeURL(bott->Getlanguage()) + "&sourceCode=" + escapeURL(bott->Getsrc());
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
    ...
    if (html.find("UserID or Password is Wrong.") != string::npos || 
            html.find("<span class=\"line\">Login</span>") != string::npos) return SUBMIT_OTHER_ERROR;
...
Bott * AizuJudger::getStatus(Bott * bott) {
    ...
    while (true) {
        ...
        prepareCurl();
        curl_easy_setopt(curl, CURLOPT_URL, ((string)"http://judge.u-aizu.ac.jp/onlinejudge/status.jsp").c_str());
        performCurl();
        ...
        if (isFinalResult(result)) {
            ...
            if (result != "Runtime Error" && result != "Compile Error") {
                if (!RE2::PartialMatch(status, "(?s)<td class=\"text-center\">([0-9\\.]*) s.*?<td.*?>([0-9]*) KB", &time_used, &memory_used)) {
            ...
            result_bott->Settime_used(intToString(stringToDouble(time_used) * 100 + 0.1));
...
string AizuJudger::getCEinfo(Bott * bott) {
    ...
    prepareCurl();
    curl_easy_setopt(curl, CURLOPT_URL, ("http://judge.u-aizu.ac.jp/onlinejudge/compile_log.jsp?runID=" + bott->Getremote_runid()).c_str());
    performCurl();
    ...
    charset_convert("SHIFT_JIS", "UTF-8//TRANSLIT", ce_info, info.length() + 1, buffer, info.length() * 2);
    
    if (!RE2::PartialMatch(buffer, "(?s)<p style=\"font-size:11pt;\">(.*)</p>", &result)) {
        return "";
    }
